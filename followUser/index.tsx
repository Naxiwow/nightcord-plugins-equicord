...
function onVoiceStateUpdates(data: any) {
    ...
    // Update followedChannel sync so future events compare against correct state
    let channelChanged = false;
    let newCh: string | null = null;
    for (const s of states) {
        if (s.userId !== followedId) continue;
        newCh = s.channelId ?? null;
    ...
    if (!channelChanged || !newCh) return;
    // Defer effects to avoid blocking Discord's audio dispatch pipeline
    setTimeout(() => {

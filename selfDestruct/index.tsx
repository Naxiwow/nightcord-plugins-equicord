...
function TimerBadge({ messageId }: { messageId: string; }) {
    ...
    React.useEffect(() => {
        function update() {
            ...
            if (!entry) { setRemaining(null); return; }
            setRemaining(Math.max(0, Math.ceil((entry.expiresAt - Date.now()) / 1000)));
        }
        update();
        const interval = setInterval(update, 1000);
        const listener = () => update();
        timerListeners.add(listener);
        return () => { clearInterval(interval); timerListeners.delete(listener); };
    }, [messageId]);
    ...
    return (
        <div style={{
            ...
            padding: "2px 8px", borderRadius: "12px", backgroundColor: "rgba(237,66,69,0.12)",
            color: "#ed4245", fontSize: "12px", fontWeight: 600,
            fontFamily: "var(--font-primary)", lineHeight: "18px",
...
function SelfDestructIcon({ active, width = 20, height = 20 }: { active?: boolean; width?: number; height?: number; }) {
    return (
        <svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg"
            width={width} height={height} fill="none" viewBox="0 0 24 24"
            ...
            {!active && (
                <path fill="var(--status-danger)" d="M21.178 1.707 22.592 3.12 4.12 21.593l-1.414-1.415L21.178 1.707Z" />
            )}
        </svg>
    );
}
...
const SelfDestructButton: ChatBarButtonFactory = ({ isMainChat }) => {
    ...
    const delayStr = delaySec >= 60
        ? `${Math.floor(delaySec / 60)}m${delaySec % 60 ? (delaySec % 60) + "s" : ""}`
        : `${delaySec}s`;

    return (
        <ChatBarButton
            tooltip={active ? `SelfDestruct: ON (${delayStr}) — click to disable` : "SelfDestruct: OFF — click to enable"}
            onClick={toggle}>
...
export default definePlugin({
    ...
    description: "Sends messages that are automatically deleted after a configurable delay.",
    ...
    flux: {
        MESSAGE_CREATE({ message, optimistic }: { message: any; optimistic?: boolean; }) {
            ...
            scheduleDelete(message.channel_id, message.id, settings.store.delay ?? 30);
        },
    },

...
async function getDeletedMessagesFromIDB(channelId: string): Promise<any[]> {
    try {
        ...
        const idb = await new Promise<IDBDatabase>((resolve, reject) => {
            ...
            dbReq.onupgradeneeded = () => { dbReq.result.close(); reject(new Error("DB not initialized")); };
        });
...
async function fetchAllMessages(channelId: string, token: string, onProgress: (n: number) => void): Promise<RichMessage[]> {
    ...
    while (true) {
        ...
        for (const m of batch) {
            messageMap.set(m.id, {
                id: m.id, timestamp: m.timestamp, editedAt: m.edited_timestamp ?? undefined,
                authorId: m.author.id, authorName: m.author.global_name ?? m.author.username,
                authorAvatar: m.author.avatar ?? null, content: m.content ?? "",
                attachments: (m.attachments ?? []).map((a: any) => ({ url: a.url, filename: a.filename, size: a.size, contentType: a.content_type ?? "application/octet-stream" })),
                embeds: (m.embeds ?? []).map((e: any) => ({ title: e.title, description: e.description, url: e.url, image: e.image?.url ?? e.thumbnail?.url, type: e.type ?? "rich" })),
                ...
                referencedMessage: m.referenced_message ? { id: m.referenced_message.id, authorName: m.referenced_message.author?.username ?? "Inconnu", content: m.referenced_message.content?.slice(0, 100) ?? "" } : undefined,
                pinned: m.pinned ?? false, type: m.type ?? 0, components: m.components ?? [], deleted: false
            });
        }
        ...
        if (cached) {
            ...
            for (const m of raw as any[]) {
                if (m && (m.deleted || m.state === "DELETED" || m.mlDeleted)) {
                    messageMap.set(m.id, {
                        id: m.id, timestamp: m.timestamp instanceof Date ? m.timestamp.toISOString() : (m.timestamp || new Date().toISOString()),
                        editedAt: m.editedTimestamp instanceof Date ? m.editedTimestamp.toISOString() : (m.editedTimestamp || m.edited_timestamp),
                        authorId: m.author?.id, authorName: m.author?.globalName ?? m.author?.username ?? "Inconnu",
                        authorAvatar: m.author?.avatar ?? null, content: m.content ?? "",
                        attachments: (m.attachments ?? []).map((a: any) => ({ url: a.url, filename: a.filename, size: a.size, contentType: a.content_type ?? "application/octet-stream" })),
                        embeds: (m.embeds ?? []).map((e: any) => ({ title: e.title, description: e.description, url: e.url, image: e.image?.url ?? e.thumbnail?.url, type: e.type ?? "rich" })),
                        stickers: (m.sticker_items ?? m.stickers ?? []).map((s: any) => ({ name: s.name, id: s.id })),
                        reactions: (m.reactions ?? []).map((r: any) => ({ emoji: r.emoji?.name ?? r.emoji?.id, count: r.count })),
                        referencedMessage: m.messageReference ? { id: m.messageReference.message_id || m.messageReference.id || "0", authorName: "Inconnu", content: "" } : undefined,
                        pinned: m.pinned ?? false, type: m.type ?? 0, components: m.components ?? [], deleted: true
                    });
                }
            }
        }
    } catch (e) { console.error("[ExportDM] Cache error:", e); }
    ...
    try {
        ...
        for (const record of idbRecords) {
            ...
            messageMap.set(record.message_id, {
                id: record.message_id, timestamp: m.timestamp ?? new Date().toISOString(),
                editedAt: m.edited_timestamp ?? undefined, authorId: m.author?.id ?? "0",
                authorName: m.author?.global_name ?? m.author?.globalName ?? m.author?.username ?? "Inconnu",
                authorAvatar: m.author?.avatar ?? null, content: m.content ?? "",
                attachments: (m.attachments ?? []).map((a: any) => ({ url: a.url ?? a.oldUrl ?? "", filename: a.filename ?? "file", size: a.size ?? 0, contentType: a.content_type ?? "application/octet-stream" })),
                embeds: (m.embeds ?? []).map((e: any) => ({ title: e.title, description: e.description, url: e.url, image: e.image?.url ?? e.thumbnail?.url, type: e.type ?? "rich" })),
                stickers: (m.sticker_items ?? m.stickerItems ?? []).map((s: any) => ({ name: s.name, id: s.id })),
                reactions: (m.reactions ?? []).map((r: any) => ({ emoji: r.emoji?.name ?? r.emoji?.id, count: r.count })),
                referencedMessage: m.referenced_message ? { id: m.referenced_message.id ?? m.referenced_message.message_id ?? "0", authorName: m.referenced_message.author?.username ?? "Inconnu", content: (m.referenced_message.content ?? "").slice(0, 100) } : undefined,
                pinned: m.pinned ?? false, type: m.type ?? 0, components: m.components ?? [], deleted: true
            });
        }
    } catch (e) { console.error("[ExportDM] IDB error:", e); }
...
function buildHtml(messages: RichMessage[], channelName: string): string {
    const rows = messages.map(m => {
        ...
        const mediaHtml = m.attachments.map(a => {
            const mtype = getMediaType(a.url, a.contentType);
            if (mtype === "image") return `<div class="media"><img src="${a.url}" alt="${a.filename}" loading="lazy"><div class="media-name">${a.filename} (${formatSize(a.size)})</div></div>`;
            if (mtype === "video") return `<div class="media"><video src="${a.url}" controls preload="none"></video><div class="media-name">${a.filename}</div></div>`;
            if (mtype === "audio") return `<div class="media"><audio src="${a.url}" controls></audio><div class="media-name">${a.filename}</div></div>`;
        ...
        const reactHtml = m.reactions.length ? `<div class="reactions">${m.reactions.map(r => `<span class="reaction">${r.emoji} ${r.count}</span>`).join("")}</div>` : "";
...
function ExportDMModal({ rootProps }: { rootProps: any; }) {
    ...
    const filtered = search.trim() ? channels.filter(c => c.name.toLowerCase().includes(search.toLowerCase())) : channels;

    return (
        <ModalRoot {...rootProps} size="medium">
            <ModalHeader separator={false}>
                <Forms.FormTitle tag="h4" style={{ margin: 0, display: "flex", alignItems: "center", gap: 8, color: "#fff" }}>
                    <ExportIcon width={16} height={16} /> Export DMs
                </Forms.FormTitle>
            ...
            <ModalContent className="edm-content">
                <div className="edm-search-bar">
                    <SearchIcon />
                    <input className="edm-search-input" type="text" placeholder="Search for a friend or a DM..."
                        value={search} onChange={e => setSearch(e.currentTarget.value)} autoFocus />
                    {search && <button className="edm-search-clear" onClick={() => setSearch("")}>✕</button>}
                </div>

                <Forms.FormTitle tag="h5" className="edm-label">CHOOSE A CONVERSATION</Forms.FormTitle>
                <div className="edm-channel-list">
                    {filtered.length === 0 && (
                        <div className="edm-empty">{search ? `No results for "${search}"` : "No DMs found"}</div>
                    )}
                    {filtered.map(c => {
                        const av = getChAvatar(c);
                        const isSel = selected.has(c.id);
                        return (
                            <div key={c.id} className={`edm-channel-row ${isSel ? "edm-channel-row--selected" : ""}`} onClick={() => toggleSelected(c.id)}>
                                {av
                ...
                <div className="edm-format-row">
                    {FORMATS.map(f => (
                        <button key={f.key} className={`edm-format-btn ${format === f.key ? "edm-format-btn--active" : ""}`}
                            onClick={() => setFormat(f.key)} title={f.desc}>
                ...
                {status !== "idle" && <div className={`edm-status edm-status--${status}`}>{progress}</div>}

                <button className="edm-export-btn" onClick={doExport} disabled={selected.size === 0 || status === "fetching"}>
